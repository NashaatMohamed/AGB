@props(['friends'])

<div class="space-y-2">
    @forelse($friends as $friend)
        <a href="{{ route('profile.show', $friend) }}"
           class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
            <!-- Avatar -->
            @if($friend->profile?->profile_picture)
                <img src="{{ Storage::url($friend->profile->profile_picture['path']) }}"
                     alt="{{ $friend->name }}"
                     class="w-12 h-12 rounded-full object-cover">
            @else
                <div class="w-12 h-12 rounded-full bg-gray-300 flex items-center justify-center text-gray-600 font-semibold">
                    {{ strtoupper(substr($friend->name, 0, 1)) }}
                </div>
            @endif

            <!-- Info -->
            <div class="flex-1 min-w-0">
                <p class="font-semibold text-gray-900 truncate">{{ $friend->name }}</p>
                @if($friend->profile?->bio)
                    <p class="text-sm text-gray-500 truncate">{{ Str::limit($friend->profile->bio, 50) }}</p>
                @endif
            </div>

            <!-- Stats -->
            <div class="text-right">
                <p class="text-xs text-gray-500">{{ $friend->posts_count }} posts</p>
            </div>
        </a>
    @empty
        <div class="text-center py-8">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>
            </svg>
            <p class="mt-4 text-gray-500">No friends yet</p>
            <a href="{{ route('friends.index') }}" class="mt-2 inline-block text-blue-600 hover:text-blue-800">
                Find friends
            </a>
        </div>
    @endforelse
</div>
